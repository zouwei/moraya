/**
 * Tauri implementation of `MediaResolver` from `@moraya/markdown-core`.
 *
 * Routes:
...
import type { MediaResolver } from '@moraya/markdown-core'
...
export class TauriMediaResolver implements MediaResolver {
  async loadLocalImage(absolutePath: string): Promise<string> {
    ...
    try {
      ...
      // Per @moraya/markdown-core §4.5 contract: resolve a fallback URL,
