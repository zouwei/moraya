...
```
git tag v* → GitHub Actions release.yml
  ├── release (现有，不变)
  │   ├── macOS aarch64 → DMG → GitHub Releases
  │   ├── macOS x86_64  → DMG → GitHub Releases
  │   ├── Windows       → MSI → GitHub Releases
  │   └── Linux         → DEB/AppImage → GitHub Releases
  │
  ├── release-ios (新增，与 release 并行)
  │   └── iPad aarch64-apple-ios
  │       ├── 签名 → 构建 IPA
  │       ├── IPA → GitHub Releases（备份下载 + 侧载）
  │       └── IPA → App Store Connect → TestFlight（自动发布外部测试者）
  │
  └── trigger-deploy (现有)
      └── needs: [release, release-ios] → Cloudflare Pages
```
...
| 资源 | 用途 | 获取方式 |
|------|------|---------|
| Apple Developer 账号 | $99/年（与 macOS 签名共用） | [developer.apple.com](https://developer.apple.com) |
| App Store Connect App | 注册 Bundle ID `com.moraya.app` | App Store Connect → My Apps → New App |
| Distribution Certificate | .p12，iOS App 签名 | Developer Portal → Certificates → Apple Distribution |
| Provisioning Profile | App Store 分发描述文件 | Developer Portal → Profiles → App Store → `com.moraya.app` |
| App Store Connect API Key | CI 自动上传 | App Store Connect → Users → Keys → Generate |

#### GitHub Secrets

| Secret | 值 | 说明 |
|--------|-----|------|
| `IOS_CERTIFICATE_BASE64` | .p12 的 base64 编码 | `base64 -i cert.p12 \| pbcopy` |
| `IOS_CERTIFICATE_PASSWORD` | .p12 导出密码 | |
| `IOS_PROVISION_PROFILE_BASE64` | .mobileprovision 的 base64 | `base64 -i profile.mobileprovision \| pbcopy` |
| `APPSTORE_API_KEY_ID` | API Key ID | 形如 `XXXXXXXXXX` |
| `APPSTORE_API_ISSUER` | API Issuer ID | UUID 格式 |
| `APPSTORE_API_PRIVATE_KEY` | .p8 文件内容 | 下载后仅可见一次 |

### Git 管理策略

`src-tauri/gen/apple/` 提交到 git（避免 CI 每次重新 init），但排除构建产物。

`.gitignore` 新增：

```gitignore
# iOS build artifacts (keep gen/apple/ project files in git)
src-tauri/gen/apple/build/
src-tauri/gen/apple/Frameworks/
src-tauri/gen/apple/*.xcworkspace/xcuserdata/
src-tauri/gen/apple/**/*.xcodeproj/xcuserdata/
...
iOS 要求每次 TestFlight 上传的 Build Number 递增。使用 `github.run_number` 自动管理：

```yaml
- name: Set iOS build number
  run: |
    # 在 Xcode 项目中设置 build number
    cd src-tauri/gen/apple
    agvtool new-version -all ${{ github.run_number }}
```

- `CFBundleShortVersionString`（版本号）= `0.7.0`（来自 `tauri.conf.json`）
- `CFBundleVersion`（构建号）= `github.run_number`（如 42, 43, 44...）
- 同一版本号可上传多次（构建号不同），TestFlight 自动显示最新

### release-ios Job 完整定义
...
release-ios:
  ...
  steps:
    ...
    # IPA 上传到 GitHub Release（与桌面包同一个 Release）
    ...
    # 上传到 App Store Connect → 自动进入 TestFlight
...
```yaml
trigger-deploy:
  needs: [release, release-ios]  # 等待桌面 + iPad 构建全部完成
```

### Release Notes 模板更新

releaseBody 新增 iPad 段落：

```markdown
### iPad

Available on TestFlight. [Join Beta →](https://testflight.apple.com/join/XXXXX)
...
```bash
pnpm version:bump 0.7.0
git add -A && git commit -m "chore: release v0.7.0"
git tag v0.7.0
git push origin main --tags
# GitHub Actions 自动完成：
#   → macOS DMG + Windows MSI + Linux DEB → GitHub Releases
#   → iPad IPA → GitHub Releases + App Store Connect → TestFlight
#   → Cloudflare Pages 部署
```
...
| 平台 | 安装方式 | 安全问题 |
|------|---------|---------|
| macOS | 下载 DMG → 拖入 Applications | 需 `xattr -cr`（未公证时） |
| Windows | 下载 MSI → 双击安装 | SmartScreen 警告（未签名时） |
| Linux | `dpkg -i .deb` 或运行 `.AppImage` | 无 |
| **iPad** | **TestFlight 一键安装 / App Store 搜索** | **无（苹果签名验证）** |
